import { el, els, getRandomID, toArray, toStr, useStorage } from './utils';
...
export const superRender = () => {
  let projects = sortProjects().reduce((previousValue, currentValue) => {
...
export const renderListItem = ({ id, title }) => {
  return `
    <li
      draggable="true"
      class="draggable  text-sm font-medium truncate dark:text-white flex space-x-4"
    >
      <div
      ...
      <div class="text-white flex space-x-4">
        <button data-draggable-id="${id}" class="project-edit-btn w-6">
          <img src="src/assets/edit.svg" alt="edit icon" />
        </button>

        <button data-draggable-id="${id}" class="w-6 project-delete-btn">
